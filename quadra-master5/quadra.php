...
<h3 class="aligncenter">Minhas Quadras</h3>

<div class="container-fluid mt-5" id="conteudoQuadra">

<div class="container-fluid">
<button class="btn btn-primary " onclick="conteudoCadQuadra()">Cadastrar Quadra</button>
</div>

<?php
  $usuarioID = $_SESSION['usuarioID'];

   $consultaQuadras = $con->prepare("SELECT id_quadra, tipo_quadra, horario_inicial, horario_final FROM quadra WHERE id_estabelecimento = '$usuarioID' ");
...
<table class="table" id="tableQuadra">
  <thead>
    <tr>
...
?>
    <tr>
      <td><span class="badge bg-warning text-dark" style="cursor:pointer;" onclick="editarQuadra('<?=$idQuadra?>')" >Editar</span><span style="cursor:pointer;" onclick="deletarQuadra('<?=$idQuadra?>')"class="badge bg-danger ml-2">Excluir</span></td>
      <td><?=$horarioIni?> às <?=$horarioFin?></td>
      <td><?=$tipoQuadra?></td>
    </tr>
...
<h5>Não existe quadra cadastrada.</h5>
</div>
<?php endif;?>
</div>
<!-- container -->
